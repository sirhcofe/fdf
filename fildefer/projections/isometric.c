...
/*   Updated: 2023/01/03 18:47:43 by chenlee          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_projected_size(t_fdf *fdf, t_map *map)
{
	...
	pixl_h = (fdf->height * 0.8) / fdf->wf.total_h;
	fdf->wf.total_w = cos(0.523599) * (map->row + map->col - 2);
	pixl_w = (fdf->width * 0.8) / fdf->wf.total_w;
	...
	fdf->wf.dot_x = pixl_w * cos(0.523599);
	fdf->wf.dot_y = pixl_h * sin(0.523599);
